...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/product/${product.id}`}>
      <div className={`group relative bg-card border border-white/5 hover:border-primary/50 transition-all duration-500 overflow-hidden cursor-pointer ${!inStock ? 'opacity-70' : ''}`}>
        {/* Out of Stock Badge */}
        ...
        <div className="p-4 md:p-6 relative">
          <div className="flex justify-between items-start mb-2">
            <div>
              <h3 className="font-display font-bold text-base md:text-xl text-white group-hover:text-primary transition-colors">
                {product.name}
              </h3>
            ...
            {!inStock && totalStock !== -1 ? (
              ...
              <div className="text-right flex-shrink-0 ml-3">
                <span className="font-display font-bold text-base md:text-lg text-white block">
          ...
          {price > 0 && inStock && (
            <Button 
              onClick={handleQuickAdd}
              size="sm"
              className="mt-3 w-full bg-primary text-black hover:bg-white font-display tracking-wider clip-corner md:hidden"
            >
