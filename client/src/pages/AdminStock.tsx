...
export default function AdminStock() {
  ...
  const updateStock = async (sku: string, newStock: number) => {
    try {
      const res = await fetch('/api/admin/catalog/stock', {
        ...
        credentials: 'include',
        body: JSON.stringify({ sku, stock: newStock }),
      });
      if (res.ok) {
        setStock(prev => prev.map(s => s.sku === sku ? { ...s, currentStock: newStock } : s));
        toast.success(`Estoque de ${sku} actualizado para ${newStock}`);
      } else {
        toast.error('Erro ao actualizar estoque');
      }
    } catch {
      toast.error('Erro ao actualizar estoque');
    }
  ...
  const totalUnits = stock.reduce((s, r) => s + r.currentStock, 0);
  const zeroStock = stock.filter(r => r.currentStock === 0).length;
  const criticalStock = stock.filter(r => r.currentStock > 0 && r.currentStock <= 2).length;
  const lowStock = stock.filter(r => r.currentStock > 0 && r.currentStock <= 5).length;

  const filtered = stock.filter(row => {
    const matchSearch = !search ||
      row.sku.toLowerCase().includes(search.toLowerCase()) ||
      row.productName.toLowerCase().includes(search.toLowerCase()) ||
      row.ecommerceColorName.toLowerCase().includes(search.toLowerCase());
    const matchCategory =
      categoryFilter === 'all' ||
      (categoryFilter === 'esportivo' && row.category === 'esportivo') ||
      (categoryFilter === 'casual' && (row.category === 'casual_masculino' || row.category === 'casual_feminino'));
    const matchStock =
  ...
  return (
    <AdminLayout title="Estoque" subtitle={`${stock.length} SKUs · ${totalUnits} unidades`}>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display font-bold text-2xl text-white tracking-wider">ESTOQUE</h1>
            <p className="font-body text-sm text-gray-500 mt-1">{stock.length} SKUs · {totalUnits} unidades totais</p>
          </div>
        ...
        <div className="flex gap-3 flex-wrap items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <Input
              placeholder="Buscar SKU, produto ou cor..."
              value={search}
        ...
        <p className="font-body text-xs text-gray-600">{filtered.length} de {stock.length} SKUs exibidos</p>
        ...
        {dataLoading ? (
          ...
          <div className="bg-[#111111] border border-white/10 overflow-hidden">
            <div className="grid grid-cols-12 gap-2 px-4 py-3 border-b border-white/10 bg-black/30">
              <div className="col-span-2 font-display font-bold text-[10px] text-gray-600 tracking-widest">SKU</div>
              <div className="col-span-4 font-display font-bold text-[10px] text-gray-600 tracking-widest">PRODUTO</div>
              <div className="col-span-3 font-display font-bold text-[10px] text-gray-600 tracking-widest">COR</div>
              <div className="col-span-1 font-display font-bold text-[10px] text-gray-600 tracking-widest text-center">LINHA</div>
            ...
            <div className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 text-gray-500">
                  ...
                  <p className="font-body text-sm">Nenhum SKU encontrado</p>
                </div>
              ) : (
                filtered.map(row => (
                  <div
                    key={row.sku}
                    className={`grid grid-cols-12 gap-2 px-4 py-3 items-center hover:bg-white/3 transition-colors ${row.currentStock === 0 ? 'bg-red-400/3' : ''}`}
                  >
                    <div className="col-span-2">
                      <span className="font-body text-xs text-gray-400 font-mono">{row.sku}</span>
                    </div>
                    <div className="col-span-4">
                      <p className="font-display font-bold text-xs text-white tracking-wider truncate">{row.productName}</p>
                    </div>
                    <div className="col-span-3">
                      <p className="font-body text-xs text-gray-400 truncate">{row.ecommerceColorName}</p>
                    </div>
                    <div className="col-span-1 text-center">
                      <span className={`font-display font-bold text-[9px] px-1.5 py-0.5 border tracking-wider ${
                        row.category === 'esportivo'
                      ...
                      {editingId === row.sku ? (
                        <div className="flex items-center gap-1">
                          <Input
                            ...
                            autoFocus
                            onKeyDown={e => {
                              if (e.key === 'Enter') updateStock(row.sku, parseInt(editValue) || 0);
                          ...
                          <Button
                            ...
                            onClick={() => updateStock(row.sku, parseInt(editValue) || 0)}
                          >
                        ...
                        <button
                          onClick={() => { setEditingId(row.sku); setEditValue(String(row.currentStock)); }}
        ...
        <p className="font-body text-xs text-gray-600 text-center">
          Clique no número de estoque para editar directamente. Pressione Enter para confirmar ou Esc para cancelar.
        </p>
      </div>
    </AdminLayout>
  );
